#!/bin/sh
# leaf4 — Lab 3 SOLVE state: overlay primitives + worker access ports on VLAN 1000.
...
# ---- SRv6 service prefix (Lab 5): the hosts behind leaf4 ----
# fd00:100:4::1 is a concrete host (a stable source/identity address you can
# ping FROM); the local /64 makes the leaf answer every address in the range, so
# pinging fd00:100:4::1..N gives the ECMP demo many distinct inner flows.
ip -6 route replace local fd00:100:4::/64 dev lo
ip -6 addr replace fd00:100:4::1/128 dev lo
...
ip -6 route replace fcbb:bb00:4:fe00:: encap seg6local action End.DT6 table 255 count dev srv6end
