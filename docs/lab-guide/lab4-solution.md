...
- **Network-wide event correlation.** When a link drops, a real telemetry pipeline correlates the BGP state change, the FRR log, the interface-down event, the dropped-packet counter spike, and the AllReduce error all in one timeline. That's what **Lab 7 (failure injection)** will build on top of this stack.
