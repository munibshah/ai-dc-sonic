...
The Lab 6 checkpoints inspect the existing healthy fabric, so they should pass against any Lab-4-solved state. When they don't, the failure is almost never about Lab 6 itself — it's about the fabric being in a non-healthy state. Top causes, in priority order:
...
| Submit ✓ runs but the ping mesh reports <56 OK | Worker eth1 IPs are not on `192.168.100.0/24` — usually means a previous Lab-1 session reset workers to /31 underlay and the orchestrator hasn't re-applied Lab-4's `solve_extra` since. (Lab 6 BOOTSTRAP doesn't run Lab 4's worker-overlay setup, so if Lab 4 wasn't the last-solved lab, workers are in the wrong state.) | Click into **Lab 4**, click **Solve**, then come back to Lab 6 |
...
Then click **Start ▶** to begin Lab 6 again, or move forward to **Lab 7** when it ships.
