...
export default function FabricHoldBanner() {
  ...
  // When the learner holds the fabric, stay quiet — no need to narrate it inside
  // the lab. Only surface the banner when actions would be locked.
  if (status.you_hold) return null;
  ...
  return (
    <div className="rounded-lg border border-amber-400/40 bg-amber-500/10 text-amber-100 px-4 py-2 text-sm flex flex-wrap items-center gap-x-2 gap-y-1">
      ...
      {status.reserved ? (
        <>Fabric reserved by another learner until {fmtUntil(status.ends_at)} — Start and Reset are locked.</>
      ) : (
        <>You don&apos;t have an active reservation — Start and Reset are locked.</>
      )}
