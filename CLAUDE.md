...
| 5 | SRv6 uSID Transport + ECMP | `_overlay_workers` + pre-provisioned IPv6 underlay (`_srv6_skeleton`) | IPv4 + EVPN-VXLAN intact, **additive** SRv6 uSID transport leaf-to-leaf (FRR locators + kernel End.DT6 endpoints + headend), per-flow flow-label ECMP across both spines (`_srv6`) | shipped |
| 6 | Super Spines — Beyond a Single-Pod CLOS | `_overlay_workers` | Same fabric — conceptual lab, inspection-only (no FRR change). See ADR-013 | shipped |
| 7 | Inject Failure During AllReduce | full overlay + AllReduce running, telemetry dashboards live | Survive a link cut mid-training, reconvergence visible in the chart | future |
...
- **`configs/frr/<state_name>/<sw>/frr.conf`** — the FRR config applied for that state. Existing states: `_skeleton` (lo + ethN interface IPs pre-provisioned, no BGP — the learner configures only BGP in Lab 1), `_canonical` (working underlay), `_overlay` (underlay + EVPN-VXLAN), `_overlay_workers` (overlay + worker access ports), `_srv6_skeleton` (Lab 5 BOOTSTRAP: `_overlay_workers` + a pre-provisioned IPv6 dual-stack underlay — `fc00:<spine>:<leaf>::/127` links + IPv6 BGP advertising per-leaf uSID locator `fcbb:bb00:<leaf>::/48`s with ECMP — but no SRv6 dataplane yet), `_srv6` (Lab 5 SOLVE: skeleton + FRR uSID locators + kernel `End.DT6` endpoints on a `srv6end` dummy + H.Encaps headend, in `overlay-setup.sh`). Lab 4 (telemetry) reuses `_overlay_workers` for both BOOTSTRAP and SOLVE — it's a procedural lab, no FRR delta. Lab 6 (super spines) also reuses `_overlay_workers` (conceptual).
...
12. **SONiC `show vxlan remotevtep` quirk** (mostly historical, see ADR-011). In the **old `netreplica/docker-sonic-vs:latest`** (2022 image, retired 2026-05-26) the swssconfig pipeline didn't back-sync FRR's EVPN-learned remote VTEPs into APP_DB, so the command returned "Total count : 0" even when the overlay worked. In **modern `aidc/sonic-vs:202511`** (current image, FRR 10.4.1) it populates correctly. Lab 2's checkpoint code falls back from `show vxlan remotevtep` to `vtysh -c "show evpn vni <vni>"` so it works against both images — keep the fallback as long as the older image is a supported revert path. **Second remotevtep quirk (current image): the `OperStatus` column reads `oper_down` for every remote VTEP even when the overlay fully works** (ping passes, FRR's view is healthy). `OperStatus` tracks the *simulated ASIC* tunnel, which never reports up on `sonic-vs`; forwarding is done by the Linux-kernel VXLAN netdev, which is fine. It's cosmetic — never gate a checkpoint on `oper_up`. The `remote_vteps_learned` check matches on the VTEP IP being *present* (any status) and falls back to FRR, so it's robust to this. The Lab 2 exercise/solution now call the `oper_down` out explicitly so learners don't think they broke something.
...
19. **SRv6 `seg6local` endpoints MUST bind to a real device — never `lo`.** Lab 5 builds SRv6 uSID endpoints (`End`/`End.DT6`) with `ip -6 route add <sid> encap seg6local action ... dev <DEV>`. With `dev lo` the kernel **silently** stores the route with NO lwtunnel attached (`ip -6 -d route show` shows a plain route, no `encap seg6local`; a uSID packet hitting it yields `ICMP destination-unreachable`). With a real device (we use a dedicated `srv6end` dummy) it attaches and decaps correctly (`ip -6 -s route show` shows `packets N`). This cost a long spike to diagnose — it was misread as "old iproute2 (6.1.0) can't do seg6local" until the same command worked on `dev eth1`. The headend (`encap seg6`, an OUTPUT lwtunnel) is unaffected by this and works on any dev. Verify endpoints functionally (ping through the path / the seg6local packet counter), not by `ip route show` alone — the display is unreliable in this image.
20. **IPv6 forwarding + seg6 are OFF by default on the sonic-vs switches.** They're IPv4 routers out of the box: `net.ipv6.conf.all.forwarding` and `...seg6_enabled` default to `0` and must be `1` for SRv6 transit/termination. Plus `net.ipv6.seg6_flowlabel=1` on the headend so the outer flow label is derived from the inner flow. bootstrap-switch.sh sets these globally now (harmless no-ops for the IPv4-only Labs 1-4). Per-interface `seg6_enabled` matters for the *ingress* interface a uSID packet arrives on (eth1/eth2 from the spines), so it's set per-link too. **The IPv6 ECMP hash twist:** `net.ipv6.fib_multipath_hash_policy` must be **`0`, not `1`** (the opposite of the IPv4/VXLAN fix in pitfall #18). For IPv6, policy 0 = "L3 + flow label" — exactly what spreads uSID traffic. Policy 1 = "L4", which on an IPv6-in-IPv6 (next-header 41) uSID packet finds no L4 ports, falls back to a constant hash, and **pins every flow to one spine**. Verified empirically: with policy=1 a 16-flow burst went 1/401 across the spines; with policy=0 it split. The flow labels were varying on the wire either way (`tcpdump -v` confirms) — the bug was the hash policy ignoring them, not the labels.
21. **FRR 10.4 takes the SRv6 locator but doesn't auto-program the kernel endpoint.** `segment-routing / srv6 / locators / locator MAIN / prefix <X>/48 block-len 32 node-len 16 / behavior usid` lands and shows under `show segment-routing srv6 locator` (Up, behavior uSID), but zebra does NOT install a kernel `End`/`uN` seg6local route from the locator alone (that needs a SID client — BGP-SRv6 services or `static-sids`, whose grammar differs and isn't wired for dataplane in this image). Lab 5 therefore programs the dataplane (endpoint + headend) directly in the kernel via `overlay-setup.sh` — the same kernel-dataplane pattern Lab 2/3 use for VXLAN (`ip link add` etc.). Note `func-bits N` is accepted but auto-derived (not echoed in running-config).
22. **Lab 2 produces Type-3 (IMET) routes ONLY — no Type-2, and that's correct.** EVPN Type-2 routes carry *host* MAC/IP bindings; Lab 2's segment has no hosts, just the leaf SVIs, and FRR doesn't advertise the SVI gateway MAC by default (`show evpn vni <vni>` → `Advertise-svi-macip: No`, `# MACs: 0`). So FRR originates only Type-3 (one "I am a VTEP for this VNI" route per leaf), which builds the HER flood list. Compounding it, the kernel VXLAN device `vtep-1000` is created **`nolearning`** (`ip -d link show vtep-1000`), so no data-plane MAC learning happens either — **every** overlay frame in Lab 2 is BUM-flooded to all VTEPs (fine at 4 leaves/1 host each). Type-2 routes and unicast MAC→VTEP fdb entries first appear in **Lab 3** when the GPU workers attach real NICs. Don't "fix" the absence of Type-2 in Lab 2, and don't write a Lab 2 checkpoint that asserts Type-2 — the old `type2_routes_present` check was renamed to `evpn_routes_learned` after it was found to be passing on Type-3 while claiming Type-2. Early drafts of the Lab 2 guide promised Type-2 routes and `oper_up`; both were corrected to match the image (verified empirically 2026-06-22).
